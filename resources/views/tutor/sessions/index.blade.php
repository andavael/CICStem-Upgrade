...
<div class="content-panel panel-enhanced">
    ...
    @if($tab === 'my')
        ...
        <form action="{{ route('tutor.sessions.index', ['tab' => 'my']) }}" method="GET" class="filter-bar">
            ...
            <a href="{{ route('tutor.sessions.index', ['tab' => 'my']) }}" class="btn btn-secondary btn-sm">Clear</a>
            @endif
        </form>
        ...
        <div class="pagination">
            {{ $mySessions->appends(['tab' => 'my', 'status' => request('status'), 'search' => request('search')])->links() }}
        </div>
        @else
    ...
    @else
        ...
        <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 16px; border-radius: 8px; margin-bottom: 20px;">
            <strong>All Scheduled Sessions:</strong> View all upcoming tutoring sessions in the system.
        </div>
        ...
        <form action="{{ route('tutor.sessions.index', ['tab' => 'all']) }}" method="GET" class="filter-bar">
            ...
            <select name="year_level" class="filter-input" onchange="this.form.submit()">
                <option value="">All Year Levels</option>
                <option value="First-Year Students" {{ request('year_level') === 'First-Year Students' ? 'selected' : '' }}>First-Year Students</option>
                <option value="Second-Year Students" {{ request('year_level') === 'Second-Year Students' ? 'selected' : '' }}>Second-Year Students</option>
            </select>
            ...
            @if(request()->hasAny(['year_level', 'search']))
            ...
            <table class="data-table enhanced-table">
                ...
                <tbody>
                    ...
                    <tr class="table-row-hover">
                        ...
                        <td>
                            @if($session->tutor)
                                <span style="font-size: 13px;">{{ $session->tutor->full_name }}</span>
                            @else
        ...
        <div class="pagination">
            {{ $allSessions->appends(['tab' => 'all', 'year_level' => request('year_level'), 'search' => request('search')])->links() }}
        </div>
        @else
        <div class="empty-state-enhanced">
            <div class="empty-icon">📋</div>
            <h3>No Sessions Found</h3>
            <p>{{ request()->hasAny(['year_level', 'search']) ? 'No sessions match your filters' : 'There are no scheduled sessions at the moment' }}</p>
            @if(request()->hasAny(['year_level', 'search']))
            <a href="{{ route('tutor.sessions.index', ['tab' => 'all']) }}" class="btn btn-secondary" style="margin-top: 20px;">Clear Filters</a>
            @endif
        </div>
        @endif
    @endif
...
<div class="content-panel panel-enhanced">
    ...
    @if($tab === 'my')
        ...
        <div class="empty-state-enhanced">
            ...
            <p>{{ request()->hasAny(['status', 'search']) ? 'Try adjusting your filters' : 'You haven\'t been assigned to any sessions yet' }}</p>
            @if(!request()->hasAny(['status', 'search']))
            <a href="{{ route('tutor.sessions.index', ['tab' => 'all']) }}" class="btn btn-primary" style="margin-top: 20px;">
